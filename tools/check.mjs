...
async function main() {
  ...
  assert(pkg.name === 'ddys-tizen', 'package name mismatch.');
  ...
  const config = await read('config.xml');
  for (const fragment of [
    ...
    assert(config.includes(fragment), `config.xml missing ${fragment}.`);
  }
  ...
  assert(!/<script[^>]+type=["']module["']/iu.test(html), 'Tizen app should not rely on ES modules.');
  ...
  for (const fragment of ['registerKey', 'getBoundingClientRect', 'scrollIntoView']) {
  ...
  for (const fragment of ['.movie-card', '.is-focused', '.player-screen', '.resource-item', '.settings']) {
  ...
  assert(workflow.includes('ddys-tizen-v0.1.0.wgt'), 'workflow artifact must include wgt.');
  ...
  for (const file of files) {
    ...
    assert(!relative.endsWith('.wgt'), 'generated wgt must not be included.');
