...
DDYS 的三星 Tizen TV Web App。它把 DDYS API 的首页、分类、搜索、详情和播放资源整理成适合电视遥控器操作的界面，并在三星电视上优先使用 `webapis.avplay` 播放。

## 功能

- 首页推荐：最新更新、热门推荐、分类入口。
- 分类浏览：电影、剧集、动漫、综艺、纪录片。
- 搜索：支持电视输入法输入片名。
- 详情页：封面、年份、类型、地区、评分、简介、资源列表。
- 播放器：Tizen `AVPlay` 优先，浏览器调试时自动使用 HTML5 video fallback。
- 遥控器：方向键移动焦点，确认键打开，返回键回首页/退出播放，彩色键快速切换搜索、收藏、历史、设置。
- 收藏：影片收藏保存在电视本地。
- 历史：记录最近播放影片和资源。
- 设置：API Base、API Key、鉴权模式、分页数量、缓存、资源过滤。
- 自检：Tizen API、遥控器 API、AVPlay、video fallback、本地存储、API 连接。
- 打包：生成 `.wgt` 和 Release ZIP，并输出 SHA-256 校验文件。
...
> 当前 `.wgt` 是可复现构建产物，不包含设备证书签名。部分真机安装仍可能需要使用自己的 Tizen 证书重新签名。
...
目标环境是三星 Tizen TV Web App。普通浏览器也可打开 `index.html` 调试界面和 API 行为，但真实电视播放能力取决于设备固件、解码器、资源格式和网络环境。
